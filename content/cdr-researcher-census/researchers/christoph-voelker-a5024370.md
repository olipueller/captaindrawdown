---
title: "Christoph Voelker"
description: "Christoph Voelker is a Senior Ocean CDR researcher at Alfred-Wegener-Institut Helmholtz-Zentrum für Polar- und Meeresforschung in DE. With 42 publications and an h-index of 4, they are among the top researchers in carbon dioxide removal."
date: 2026-07-11T12:53:41.631353
url: "/cdr-researcher-census/researchers/christoph-voelker-a5024370/"
layout: "researcher"
hiddenInHomeList: true
robots: "index, follow"
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Person",
  "name": "Christoph Voelker",
  "url": "https://www.captaindrawdown.com/cdr-researcher-census/researchers/christoph-voelker-a5024370/",
  "affiliation": {
    "@type": "Organization",
    "name": "Alfred-Wegener-Institut Helmholtz-Zentrum f\u00fcr Polar- und Meeresforschung"
  },
  "sameAs": [
    "https://orcid.org/0000-0003-3032-114X",
    "https://openalex.org/A5024370682"
  ]
}
</script>

## Profile

**Christoph Voelker**  
Alfred-Wegener-Institut Helmholtz-Zentrum für Polar- und Meeresforschung · 🇩🇪 DE

**Career Stage:** Senior

## CDR Specialization

**Ocean CDR**

Leveraging ocean alkalinity, nutrient cycling, or seaweed farming to sequester carbon in the ocean.

## Metrics

| Metric | Value |
|--------|-------|
| h-index | 4 |
| Citations | 104 |
| Publications | 42 |
| CDR Focus | 16.7% |
| Trajectory | Declining |

## Top CDR Publications

1. **Climate change driven effects on transport, fate and biogeochemistry of trace element contaminants in coastal marine ecosystems** (2024)
   88 citations

2. **Alkalinity biases in CMIP6 Earth System Models and implications for simulated CO2 drawdown via artificial alkalinity enhancement** (2023)
   17 citations · General CDR

3. **Alkalinity biases in CMIP6 Earth system models and implications for simulated CO <sub>2</sub> drawdown via artificial alkalinity enhancement** (2023)
   15 citations · General CDR

4. **Interactions between ocean alkalinity enhancement and phytoplankton in an Earth system model** (2025)
   3 citations · General CDR

5. **Global sensitivity analysis of a one-dimensional ocean biogeochemical model** (2023)
   3 citations · General CDR

6. **Interactions between ocean alkalinity enhancement and phytoplankton in an Earth System Model** (2025)
   1 citations

7. **Alkalinity and Sensitivity to Alkalinity Enhancement in CMIP6 Earth System Models** (2023)
   1 citations · Ocean CDR

8. **Constraining glacial ocean carbon cycle &amp;#8211; A multi-model study** (2024)
   0 citations · Ocean CDR

9. **New / improved model parametrizations for responses in phytoplankton growth and calcification to changes in alkalinity implemented** (2023)
   0 citations · Ocean CDR

## External Profiles

- [ORCID](https://orcid.org/0000-0003-3032-114X)
- [OpenAlex](https://openalex.org/A5024370682)

---

**Part of the [CDR Researcher Census](/cdr-researcher-census/)** — [Browse all researchers](/cdr-researcher-census/researchers/) · [Search & filter](/cdr-researcher-census/lookup/)
