---
title: "Alexander C. Forse"
description: "Alexander C. Forse is an Eminent Ocean CDR researcher at University of Cambridge in GB. With 199 publications and an h-index of 43, they are among the top researchers in carbon dioxide removal."
date: 2026-07-10T00:49:19.911599
url: "/cdr-researcher-census/researchers/alexander-c-forse-a5005354/"
layout: "researcher"
hiddenInHomeList: true
robots: "index, follow"
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Person",
  "name": "Alexander C. Forse",
  "url": "https://www.captaindrawdown.com/cdr-researcher-census/researchers/alexander-c-forse-a5005354/",
  "affiliation": {
    "@type": "Organization",
    "name": "University of Cambridge"
  },
  "sameAs": [
    "https://orcid.org/0000-0001-9592-9821",
    "https://openalex.org/A5005354876"
  ]
}
</script>

## Profile

**Alexander C. Forse**  
University of Cambridge · 🇬🇧 GB

**Career Stage:** Eminent

## CDR Specialization

**Ocean CDR**

Leveraging ocean alkalinity, nutrient cycling, or seaweed farming to sequester carbon in the ocean.

## Metrics

| Metric | Value |
|--------|-------|
| h-index | 43 |
| Citations | 8,848 |
| Publications | 199 |
| CDR Focus | 5.0% |
| Trajectory | Declining |

## Top CDR Publications

1. **Capturing carbon dioxide from air with charged-sorbents** (2024)
   166 citations

2. **Carbon Dioxide Capture at Nucleophilic Hydroxide Sites in Oxidation‐Resistant Cyclodextrin‐Based Metal–Organic Frameworks**** (2022)
   55 citations

3. **Carbon Capture from Natural Gas Flue Emissions and Air via (Bi)Carbonate Formation in a Cyclodextrin-Based Metal–Organic Framework** (2025)
   12 citations · DAC

4. **Carbon Dioxide Capture at Nucleophilic Hydroxide Sites in Oxidation‐Resistant Cyclodextrin‐Based Metal–Organic Frameworks**** (2022)
   10 citations

5. **Carbon Dioxide Capture at Nucleophilic Hydroxide Sites in Oxidation-Resistant Cyclodextrin-Based Metal-Organic Frameworks** (2022)
   4 citations

6. **Carbon Capture from Natural Gas Flue Emissions and Air via (Bi)Carbonate Formation in a Cyclodextrin-Based Metal-Organic Framework** (2024)
   3 citations

7. **Capturing Carbon Dioxide from Air with Charged-Sorbents** (2024)
   3 citations

8. **Developing Joule-heating regeneration of carbon-based adsorbents for Direct Air Capture** (2025)
   2 citations

9. **Demonstrating Electrochemical CO$_2$ Capture on Redox-Active Metal-Organic Frameworks** (2024)
   0 citations · Ocean CDR

10. **Capturing Carbon Dioxide from Air with Charged Sorbents** (2023)
   0 citations

## External Profiles

- [ORCID](https://orcid.org/0000-0001-9592-9821)
- [OpenAlex](https://openalex.org/A5005354876)

---

**Part of the [CDR Researcher Census](/cdr-researcher-census/)** — [Browse all researchers](/cdr-researcher-census/researchers/) · [Search & filter](/cdr-researcher-census/lookup/)
