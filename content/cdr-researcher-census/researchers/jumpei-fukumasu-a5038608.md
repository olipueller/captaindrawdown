---
title: "Jumpei Fukumasu"
description: "Jumpei Fukumasu is a Mid-career Soil Carbon researcher at National Agriculture and Food Research Organization in JP. With 26 publications and an h-index of 7, they are among the top researchers in carbon dioxide removal."
date: 2026-07-11T12:53:40.869568
url: "/cdr-researcher-census/researchers/jumpei-fukumasu-a5038608/"
layout: "researcher"
hiddenInHomeList: true
robots: "index, follow"
---

<script type="application/ld+json">
{
  "@context": "https://schema.org",
  "@type": "Person",
  "name": "Jumpei Fukumasu",
  "url": "https://www.captaindrawdown.com/cdr-researcher-census/researchers/jumpei-fukumasu-a5038608/",
  "affiliation": {
    "@type": "Organization",
    "name": "National Agriculture and Food Research Organization"
  },
  "sameAs": [
    "https://orcid.org/0000-0002-4875-1941",
    "https://openalex.org/A5038608780"
  ]
}
</script>

## Profile

**Jumpei Fukumasu**  
National Agriculture and Food Research Organization · 🇯🇵 JP

**Career Stage:** Mid-career

## CDR Specialization

**Soil Carbon**

Enhancing carbon storage in agricultural and terrestrial soils through management practices and biochar amendment.

## Metrics

| Metric | Value |
|--------|-------|
| h-index | 7 |
| Citations | 252 |
| Publications | 26 |
| CDR Focus | 19.2% |
| Trajectory | Stable |

## Top CDR Publications

1. **Relations between soil organic carbon content and the pore size distribution for an arable topsoil with large variations in soil properties** (2022)
   114 citations · Soil Carbon

2. **Approaches to delineate aggregates in intact soil using X-ray imaging** (2021)
   45 citations

3. **Links between soil pore structure, water flow and solute transport in the topsoil of an arable field: Does soil organic carbon matter?** (2024)
   25 citations · Soil Carbon

4. **Supplementary material to "O <sub>2</sub> :CO <sub>2</sub> exchange ratio and isotopic composition of atmospheric O <sub>2</sub> measured using soil chambers, and their application to evaluating enhanced rock weathering and the Dole–Morita effect"** (2026)
   0 citations · Enhanced Weathering

5. **O <sub>2</sub> :CO <sub>2</sub> exchange ratio and isotopic composition of atmospheric O <sub>2</sub> measured using soil chambers, and their application to evaluating enhanced rock weathering and the Dole–Morita effect** (2026)
   0 citations · Enhanced Weathering

6. **Links between Soil Pore Structure, Water Flow and Solute Transport in the Topsoil of an Arable Field: Does Soil Organic Carbon Matter?** (2024)
   0 citations · Soil Carbon

7. **Is it possible to delineate aggregates in X-ray images of intact soil samples?** (2022)
   0 citations · Soil Carbon

## External Profiles

- [ORCID](https://orcid.org/0000-0002-4875-1941)
- [OpenAlex](https://openalex.org/A5038608780)

---

**Part of the [CDR Researcher Census](/cdr-researcher-census/)** — [Browse all researchers](/cdr-researcher-census/researchers/) · [Search & filter](/cdr-researcher-census/lookup/)
